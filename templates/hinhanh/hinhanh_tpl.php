    ...
    <?php if(isset($news) && count($news) > 0) { for($i=0;$i<count($news);$i++) { ?>
        <div>
            <a class="" data-fancybox="gallery" data-src="<?=UPLOAD_NEWS_L.$news[$i]['photo']?>" title="<?=$news[$i]['ten'.$lang]?>">
                <p><img onerror="this.src='<?=THUMBS?>/480x360x2/assets/images/noimage.png';" src="<?=THUMBS?>/320x240x1/<?=UPLOAD_NEWS_L.$news[$i]['photo']?>" alt="<?=$news[$i]['ten'.$lang]?>" alt="<?=$news[$i]['ten'.$lang]?>"/></p>                
            </a>
